...
function containsQueryParams(container: Params, containee: Params): boolean {
  return (
    ...
    Object.keys(containee).every((key) => equalArraysOrString(container[key] as string[], containee[key] as string[]))
  );
}
...
export class DefaultUrlSerializer implements UrlSerializer {
  ...
  parse(url: string): UrlTree {
    const _url = MatrixRouteHelper.decrypt(url);
    const p = new UrlParser(_url);
...
export function serializePath(path: UrlSegment): string {
  return `${path.path.split('/').map(encodeUriSegment).join('/')}${serializeMatrixParams(path.parameters)}`;
}
...
function serializeMatrixParams(params: { [key: string]: string }): string {
  ...
  return MatrixRouteHelper.encryption(stringParams);
}
