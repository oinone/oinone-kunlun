...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const change = (val: BigNumber | number | string | null | undefined): boolean => {
    ...
    const setValue = (val: BigNumber | number | string | null | undefined): boolean => {
      if (isNil(val)) {
        return change(val);
      }
      if (isString(val)) {
  ...
  render() {
    ...
    return createVNode(
      ...
      {
        value: fv,
        // type=number 在ios上会导致输入多个小数点后失焦事件异常, inputmode=numeric 会限制输入格式
        type: 'text',
        inputmode: this.inputmode || IInputmodeEnum.NUMERIC,
