...
import { DslRender, DslRenderWidget, DslRenderWidgetProps, Widget, InnerWidgetType } from '@kunlun/vue-widget';
...
MobileViewWidget.Selector = (options) => {
  const { tagName } = options;
  let maskOptions: IMobileViewFilterOptions;
  if (tagName) {
    maskOptions = {
      ...options,
      dslNodeType: tagName
...
export class BaseMaskWidget<Props extends BaseMaskWidgetProps = BaseMaskWidgetProps> extends DslRenderWidget<Props> {
   ...
   * 获取url参数
   */
  public getUrlParameters(): Record<string, string> {
