...
> extends BaseDataWidget<Props> {
  ...
  // 部分组件需要强制上下布局，如：富文本、上传
  @Widget.Reactive()
  public get fakeVertical() {
    return false;
  }

  // 字段右侧是否有可点击的向右的箭头icon
  @Widget.Reactive()
  public get isLink() {
    return false;
  }

  // formItem的content区域是否自适应高度，字段内的table和form可设置
  @Widget.Reactive()
  public get isAutoHeight() {
    return false;
  }

  // 底部是否有边框，基本类型的默认有这条线，table和form不需要展示
  @Widget.Reactive()
  ...
  public get showAllowClear() {
    return !(this.disabled || this.readonly) && this.allowClear && !isEmptyKeObject(this?.value);
  }
