...
import type { PopupEventHandle, PopupEventHandles } from '../../typing';
...
{
  protected handlers: PopupEventHandle = { cancel: [], ok: [] };
  ...
  public async onVisibleChange<Action extends RuntimeAction = RuntimeAction>(visible: boolean, triggerAction?: Action) {
    ...
    if (visible) {
      if (!popupInstance) {
        ...
        PopupManager.INSTANCE.push(popupInstance);
      }
    }
    if (popupInstance) {
  ...
  protected onNotify(type: keyof PopupEventHandles) {
  ...
  @Widget.Reactive()
  protected currentRootData: ActiveRecord[] | undefined;
  ...
  @Widget.Reactive()
  protected currentDataSource: ActiveRecord[] | null | undefined;
  ...
  @Widget.Reactive()
  protected currentActiveRecords: ActiveRecord[] | undefined;
