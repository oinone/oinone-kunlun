...
import { type OioTreeNode, StringHelper, uniqueKeyGenerator } from '@oinone/kunlun-shared';
...
import type { SimpleTreeModelField, TreeData, TreeNodeMetadata } from '../typing';
...
export class TreeUtils {
  ...
  public static convert(dsl: DslDefinition | undefined): TreeNodeMetadata | undefined {
    ...
    const appendChild = (widget: DslDefinition) => {
      const { model, title, label, labelFields, searchFields, references, selfReferences, filter, search } = widget;
      if (!model) {
        return;
      }
  ...
  public static async consumerReferenceModelField(
    ...
    consumer: (originFields: string[], targetFields: string[], index: number) => void,
    consumerBefore?: () => void
  ): Promise<boolean> {
          ...
          case ModelFieldType.ManyToOne:
            if (originModel === references) {
              for (let i = 0; i < length; i++) {
                consumer(referenceFields, relationFields, i);
              }
            } else {
              for (let i = 0; i < length; i++) {
                consumer(relationFields, referenceFields, i);
              }
            }
            break;
