import { executeViewAction, RedirectTargetEnum, type RuntimeViewAction } from '@oinone/kunlun-engine';
import { type IModel, type IModule, type IViewAction, ViewActionTarget, ViewType } from '@oinone/kunlun-meta';
...
export const onJumpModelDesigner = async (module: IModel, modelDesignerDisplayType: 'list' | 'graph' = 'list') => {
  ...
  if (action) {
    executeViewAction(
      ...
      undefined,
      undefined,
      { appSwitcherName: '模型设计器', hideMenu: true, module: action.moduleName },
...
export const onJumpWorkflowDesigner = async (moduleModule) => {
  ...
  if (action) {
    executeViewAction(
      ...
      undefined,
      undefined,
      { appSwitcherName: '流程设计器', hideMenu: true },
      RedirectTargetEnum.BLANK
    );
  }
};
...
export const onJumpUiDesignerHomePage = async (module: IModule, isJumpHomepage?: boolean) => {
  ...
  if (action) {
    executeViewAction(
      ...
      undefined,
      undefined,
      { appSwitcherName: '界面设计器', hideMenu: true, createViewSource: 'HOME_PAGE', module: 'uiDesigner' },
      RedirectTargetEnum.BLANK
    );
  }
};
...
export const onJumpCodeFuse = async (id) => {
  ...
  if (action) {
    executeViewAction(
      ...
      undefined,
      undefined,
      { initQueryParams: JSON.stringify({ moduleDefinition: { id } }), module: action.moduleName },
      RedirectTargetEnum.BLANK
    );
  }
};
