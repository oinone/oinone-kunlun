...
import { createVNode, defineComponent, ref, type Slot, type VNode, VNodeProps } from 'vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // const removeGhost = (sendEvent, originalEvent: DragEvent) => {
    //   const targetNode = (originalEvent.target as HTMLElement).nextSibling;
    //   if (targetNode) {
    ...
    const move = (sendEvent: DraggableSendEvent, originalEvent: DragEvent) => {
      ...
      if (result === false) {
        if (ghostBody.value) {
          // removeGhost(ghostBody.value.sendEvent, ghostBody.value.originalEvent);
    ...
    const onChange = ({ added, moved, removed }) => {
  ...
  render() {
    const item: Slot | undefined = this.$slots.item;
