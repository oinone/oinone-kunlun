...
import { OioSelect, SelectItem, useInjectOioDefaultFormContext } from '@oinone/kunlun-vue-ui-mobile-vant';
...
export default defineComponent({
  ...
  render() {
    ...
    return createVNode(
      ARadioGroup,
      {
        class: 'mobile-tab-select search-tab-select',
        value: realValue,
        disabled
      },
      {
        default: () => {
          return finalOptions.map((option) => {
            return createVNode(
              ARadioButton,
              {
                class: 'tab-select-item search-tab-select-item',
                value: option.value,
