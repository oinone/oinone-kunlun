<template>
  <div class="mobile-default-icon-select">
    <div v-if="isSelected" class="mobile-default-icon-select-icon">
      <oio-icon :icon="value" size="18px" color="var(--oio-primary-color)" />
    </div>
    <oio-button @click="showModal">{{ translateValueByKey('选择图标') }}</oio-button>
    <oio-modal
      :data="editValue"
      v-model:visible="visible"
      :loading="loading"
      :confirm-loading="loading"
      :title="modalTitle || translateValueByKey('选择图标')"
      width="small"
