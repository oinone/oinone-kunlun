...
> extends FormNumberAbstractFieldWidget<Value> {
  ...
  public validator(): Promise<ValidatorInfo> {
    return super.validator().then((res) => {
      ...
      const { value } = this;
      return this.checkValue(value, res);
    });
  }
   ...
   */
  protected computeNumericFieldMax(size, precision): string {
    if (isNil(size)) {
  ...
  protected computeNumericFieldMin(size, precision): string {
    if (isNil(size)) {
  ...
  protected _computeNumericFieldMax(size, precision): BigNumber {
