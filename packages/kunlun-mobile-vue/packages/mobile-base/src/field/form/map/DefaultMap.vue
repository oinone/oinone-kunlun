<template>
  <div class="mobile-field-map-form">
    ...
    <template v-else>
      <oio-button @click="addRecord" type="primary" :disabled="limit && items.length >= limit">{{ translateValueByKey('添加') }}</oio-button>
      <oio-form style="display: flex; flex-wrap: wrap; align-items: center">
        <oio-form-item v-for="(item, index) in items" :key="index" hide-label no-border-bottom
          ...
          <div style="display: flex; align-items: center">
            ...
            <span class="van-icon van-icon-clear van-field__clear" style="margin-left: 8px"
              @click="removeRecord(index)"></span>
          </div>
        </oio-form-item>
      </oio-form>
    </template>
...
import { OioButton, OioForm, OioFormItem, OioInput } from '@kunlun/vue-ui-mobile-vant';
