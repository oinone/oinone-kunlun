...
> extends BaseElementViewWidget<Props> {
  ...
  public generatorCondition(condition?: Condition): Condition | undefined {
    ...
    const context = this.refreshConditionContext();
    const { filter, domain } = this;
    let result = concatCondition(filter, domain, condition);
    if (context) {
      if (!result) {
        result = new Condition(DEFAULT_TRUE_CONDITION);
      }
      result.setConditionBodyData({
        ...(result.getConditionBodyData() || {}),
        ...context
      });
    }
    return result;
