import { ExperimentalConfigManager, parseConfigs, RelationUpdateType, ROOT_HANDLE, SubmitType, SubmitValue } from '@oinone/kunlun-engine';
...
import { type ActiveRecordsWidgetProps, Widget, type WidgetSubjection } from '@oinone/kunlun-vue-widget';
...
> extends BaseElementWidget<Props> {
  ...
  protected refreshCondition() {
    let mainViewFilter: string | undefined;
    if (this.inline) {
      if (this.metadataRuntimeContext.parentContext?.handle !== ROOT_HANDLE) {
        mainViewFilter = this.metadataRuntimeContext.view.filter;
      }
    } else {
      mainViewFilter = this.metadataRuntimeContext.view.filter;
    }
