<template>
  <div class="default-table-footer-operator">
    ...
    <!-- 快速填报 -->
    <element-tag widget="QuickFill" v-if="showQuickFill"></element-tag>
  </div>
</template>
...
export default defineComponent({
  name: 'DefaultTableFooterOperator',
  props: {
    showAddBtn: {
      type: Boolean,
      default: false
...
<style lang="scss">
.default-table-footer-operator {
  text-align: left;
  position: relative;
  width: 100%;
  color: var(--oio-primary-color);
  top: calc(var(--oio-row-gap) / 2 * -1);
  display: flex;
  column-gap: var(--oio-row-gap);

  .default-table-footer-operator-add-one {
    cursor: pointer;
    display: flex;
    align-items: center;
    column-gap: 4px;
