...
function collectionAffectiveChildren(dsl: DslDefinition, children: DslDefinition[]) {
  for (const widget of dsl.widgets || []) {
    ...
    if (DslDefinitionHelper.isTemplate(widget)) {
      if (widget.slot === DEFAULT_SLOT_NAME) {
        collectionAffectiveChildren(widget, children);
      }
      continue;
    }
...
function isOnlyOneRowWidget(dsl: DslDefinition) {
  const children: DslDefinition[] = collectionAffectiveChildren(dsl, []);
