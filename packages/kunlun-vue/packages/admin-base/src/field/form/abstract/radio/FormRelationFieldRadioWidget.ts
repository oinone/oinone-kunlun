...
export abstract class FormRelationFieldRadioWidget<
...
> extends FormComplexFieldWidget<ActiveRecord, Field> {
   ...
   */
  @Widget.Method()
  public onSelectedChange(key: string | undefined) {
  ...
  public async validator(): Promise<ValidatorInfo> {
    ...
    if (!selectedValue && this.required && !this.invisible) {
  ...
  protected backfillSelectedValues() {
    ...
    const key = this.generatorKey(value);
    if (!key) {
      console.warn('record pk is null.', value);
      return;
    }
    this.selectedValue = key;
  }
