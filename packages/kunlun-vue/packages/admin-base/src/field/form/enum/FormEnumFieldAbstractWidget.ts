...
> extends FormFieldWidget<Value, RuntimeEnumerationField> {
  ...
  @Widget.Reactive()
  private internalSelectedOptions: string[] = [];
    ...
    if (Array.isArray(value)) {
      ...
      if (this.field.multi && (this.required || length > 0)) {
        if (this.required && !this.invisible) {
          ...
          const optionValueList = this.options.map((_o) => _o.name);
  ...
  protected getAvailableOptions(): RuntimeEnumerationOption[] {
    ...
    for (const child of configOptions) {
      ...
      if (realInvisible === undefined || !realInvisible) {
        ...
        if (displayName?.toString() === 'NaN' && displayName !== oneOption.displayName) {
          ...
          displayName && (oneOption.displayName = translateValueByKey(displayName));
        }
        if (oneOption.label) {
          const label = this.executeExpression(oneOption.label, oneOption.label);

          if (label?.toString() === 'NaN' && label !== oneOption.label) {
            // do nothing.
          } else {
            label && (oneOption.label = translateValueByKey(label));
          }
        }
    ...
    const value = (((this.field.multi ? this.value : [this.value]) || []) as string[]).filter((v) => v);

    if (!this.internalSelectedOptions.length && value.length) {
      this.internalSelectedOptions = value;
    }
    xmlOptions = xmlOptions.filter((opt) => {
      if (!this.internalSelectedOptions.includes(opt.name) && opt.state === EnumOptionState.INACTIVE) {
        return false;
      }
      return true;
    });
