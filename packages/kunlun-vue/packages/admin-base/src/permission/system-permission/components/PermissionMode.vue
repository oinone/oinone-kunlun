<template>
  <div class="permission-mode">
    <oio-button :type="permissionMode === IPermissionModeEnum.BASE ? 'primary' : 'default'" @click="changePermissionMode(IPermissionModeEnum.BASE)">{{ translateValueByKey('基础管理模式') }}</oio-button>
    <oio-button :type="permissionMode === IPermissionModeEnum.VIEW ? 'primary' : 'default'" @click="changePermissionMode(IPermissionModeEnum.VIEW)">{{ translateValueByKey('权限查阅模式') }}</oio-button>
  </div>
</template>
