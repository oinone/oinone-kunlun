...
function buildMetadataGQLParameter(
  ...
  return `${key ? `${key}: ` : ''} {
      ...
      ${GraphqlHelper.buildStringGQLParameter('icon', metadata.icon)}
      ${GraphqlHelper.buildNotStringGQLParameter('translate', metadata.translate)}
      ${
      ...
      ${GraphqlHelper.buildNotStringGQLParameter('expandEndLevel', metadata.expandEndLevel)}
    }`;
}
...
`;
  ...
  public static async fetchExpandEndLevel(
    metadataList: TreeNodeMetadata[],
    options?: {
      expressionParameters?: ExpressionRunParam;
    }
...
`;
    const result = await http.query<TreeNodeResponseBody[]>(SYSTEM_MODULE_NAME.BASE, gql);
    return result.data.uiTreeNodeQuery.fetchExpandEndLevel;
  }
...
`;
  ...
  public static async reverselyQueryWithSize(
    ...
    options?: {
      pageSize?: number;
      expressionParameters?: ExpressionRunParam;
      disabledIsLeaf?: boolean;
    }
  ): Promise<TreeNodeResponseBody[]> {
    return GQL.query('uiTreeNodeQuery', 'reverselyQueryWithSize')
      .buildRequest((builder) =>
        builder
          .numberParameter('size', options?.pageSize || 50)
          .objectParameter(
            'leafNodes',
            `[${values.map((v) => `{ value: "${GraphqlHelper.serializableObject(v)}" }`).join(', ')}]`
          )
      ...
      .buildResponse((builder) => {
        builder.parameter('label', 'value', 'key', 'parentKeys', 'metadataKey');
        if (!options?.disabledIsLeaf) {
          builder.parameter('isLeaf');
        }
      })
      .request(SYSTEM_MODULE_NAME.BASE);
  }
  ...
  public static convertTreeByResponseBody<V extends TreeData = TreeData>(
    ...
    options: {
      ...
      generatorKey?(item: TreeNodeResponseBody, value: V): string;
      computeNodeTitle(value: V): string;
    }
  ): { nodes: OioTreeNode<V>[]; expandedKeys: string[] } {
    const expandedKeys: string[] = [];
    const isUsingPagination = !!options.defaultPagination;
    const generatorKey = options.generatorKey || ((item) => item.key || uniqueKeyGenerator());
    const nodes = CastHelper.cast<OioTreeNode<V>[]>(
      TreeHelper.convert<TreeNodeResponseBody, V, OioTreeNode<V>>(
        list,
        (v, value) => {
          const key = v.key || generatorKey(v, value);
